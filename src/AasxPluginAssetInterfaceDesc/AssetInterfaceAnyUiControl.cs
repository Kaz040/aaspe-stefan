...
namespace AasxPluginAssetInterfaceDescription
{
    public class AssetInterfaceAnyUiControl
    {
            ...
            Aas.Submodel sm)
        {
            ...
            var grid = view.Add(uitk.AddSmallGrid(rows: 6, cols: 2, colWidths: new[] { "110:", "*" }));
            ...
            //To solve Stefan's Use-case. will be removed in original verison

            AnyUiUIElement.RegisterControl(
                 uitk.AddSmallButtonTo(grid, 2, 1,
                     margin: new AnyUiThickness(2), setHeight: 21,
                     padding: new AnyUiThickness(2, 0, 2, 0),
                     content: "execute binding service"),
                 (o) =>
                 {
                     try
                     {
                         ...
                         // Create Bindings
                         _allInterfaceStatus?.HandleBindingService();
            ...
            AnyUiUIElement.RegisterControl(
                uitk.AddSmallButtonTo(grid, 5, 1,
                    margin: new AnyUiThickness(2), setHeight: 21,
                    padding: new AnyUiThickness(2, 0, 2, 0),
            ...
            List<AidInterfaceStatus> interfaces)
        {
            ...
            // ok
            var grid = view.Add(uitk.AddSmallGrid(rows: interfaces.Count + 1, cols: 5, 
            ...
            foreach (var ifx in interfaces)
            {
                ...
                var bindingcolor = StoredPrint.LightThemeColor(ifx.BindingColor);

                var binding = uitk.Set(
                    uitk.AddSmallBasicLabelTo(grid, rowIndex++, 0, fontSize: 0.8,
                        verticalAlignment: AnyUiVerticalAlignment.Center,
                        verticalContentAlignment: AnyUiVerticalAlignment.Center,
                        background: new AnyUiBrush(bindingcolor.Item2),
                        foreground: new AnyUiBrush(bindingcolor.Item1),
                        margin: new AnyUiThickness(0, 4, 0, 8),
                        padding: new AnyUiThickness(2, 1, 2, 1),
                        content: "" + ifx.BindingService),
                    colSpan: 5);
